...
describe('SwedishPlusgiroValidation', () => {
    ...
    describe('when validating', () => {
        it('should invalidate a plusgiro account number when not passing mod 10 sum check', () => {
            chai.expect(
                validation.validate({ accountNumber: '4-3', countryCode: 'SE' })
            ).to.deep.equal({
                valid: false,
        ...
        it('should validate a plusgiro account number with format X-Y', () => {
            chai.expect(
                validation.validate({ accountNumber: '4-2', countryCode: 'SE' })
            ).to.deep.equal({
                valid: true,
                reason: null,
            })

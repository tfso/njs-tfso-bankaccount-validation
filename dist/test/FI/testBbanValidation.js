...
const FI_BbanValidation_1 = require("../../src/validators/FI_BbanValidation");
describe('FI_BbanValidation', () => {
    let validation;
    beforeEach(() => {
        validation = new FI_BbanValidation_1.FI_BbanValidation({});
    });
    describe('when validating', () => {
        it('should validate a valid German BBAN account as object input', () => {
            chai.expect(validation.validate({ accountNumber: '12345600000785' })).to.deep.equal({
                valid: true,
                reason: null
            });
        });
        it('should invalidate an invalid Danish BBAN account', () => {
            chai.expect(validation.validate({ accountNumber: '1234560000078' })).to.deep.equal({
                valid: false,
    ...
    describe('when determining suitability', () => {
        it('should accept a finnish country', () => {
            chai.expect(validation.canValidate({ accountNumber: 'invalid', countryCode: 'FI', type: 'bban' })).to.equal(true);
        });
